...
def calc_latency(device_list):
    ...
    for each_device in device_list:
        if each_device.connect_stations != []:
            ...
            if each_device.connect_stations[0][1] > maximum:
                maximum = each_device.connect_stations[0][1]
            elif each_device.connect_stations[0][1] < minimum:
                minimum = each_device.connect_stations[0][1]
        quantity += 1
    for each_device in device_list:
        if each_device.connect_stations != []:
            normalization += (each_device.connect_stations[0][1] - minimum) / (maximum - minimum)
